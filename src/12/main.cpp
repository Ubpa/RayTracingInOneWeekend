...
Ptr<Hitable> GenScene() {
	...
	for (int a = -11; a < 11; a++) {
		for (int b = -11; b < 11; b++) {
			...
			Vec3f center(a + 0.9*Util::RandF(), 0.2f, b + 0.9*Util::RandF());
			if ((center - Vec3f(4, 0.2, 0)).Norm() > 0.9f) {
				Ptr<Material> material;
				if (choose_mat < 0.8f) // diffuse
					material = Lambertian::New(Vec3f(Util::RandF()*Util::RandF(), Util::RandF()*Util::RandF(), Util::RandF()*Util::RandF()));
				else if (choose_mat < 0.95f) // metal
					material = Metal::New(Vec3f(0.5f*(1 + Util::RandF()), 0.5f*(1.f + Util::RandF()), 0.5f*(1.f + Util::RandF())), 0.5f*Util::RandF());
				else {  // glass
	...
	scene->Add(Sphere::New(Vec3f(0, 1, 0), 1.0, Dielectric::New(1.5)));
	scene->Add(Sphere::New(Vec3f(-4, 1, 0), 1.0, Lambertian::New(Vec3f(0.4, 0.2, 0.1f))));
	scene->Add(Sphere::New(Vec3f(4, 1, 0), 1.0, Metal::New(Vec3f(0.7, 0.6, 0.5), 0.0f)));
